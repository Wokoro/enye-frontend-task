...
export const filterProfiles = (data, paymentType = '', cardType = '') => {
  ...
  if (!paymentType && !cardType) return data;

  if (paymentType && !cardType) return data.filter((profile) => paymentReg.test(profile.PaymentMethod));

  if (!paymentType && cardType) return data.filter((profile) => cardReg.test(profile.CreditCardType));

  return data.filter(profile => {
    return (paymentReg.test(profile.PaymentMethod) || cardReg.test(profile.CreditCardType));
  });
};